...
public class UrlValidatorTest extends TestCase {
	...
	public void testManualTest() {
		...
		assertNoExit("Null URL", "false", urlvalidator, null);
		assertNoExit("Short URL", "true", urlvalidator, "http://0.com");

		// Testing SCHEME: per apache commons documentation, it will validate http,
		// https, and ftp

		assertNoExit("URL with http", "true", urlvalidator, "http://www.test.com/");

		assertNoExit("URL with https", "true", urlvalidator, "https://www.test.com/");

		assertNoExit("URL with ftp", "true", urlvalidator, "ftp://www.test.com/");

		assertNoExit("URL with 00 prefix", "false", urlvalidator, "00://www.test.com/");
		assertNoExit("URL with 0a prefix", "false", urlvalidator, "0a://www.test.com/");
		assertNoExit("URL with path 1", "true", urlvalidator, "http://www.test.com/a");
		assertNoExit("URL with path 2", "true", urlvalidator, "http://www.test.com/a/a");
		assertNoExit("URL with query", "true", urlvalidator, "http://www.test.com/a?docid=1");
		assertNoExit("URL with fragment", "true", urlvalidator, "http://www.test.com/a?docid=1#5");
		assertNoExit("URL with spaces", "false", urlvalidator, "http://www.example.com/ %20 .html");
		assertNoExit("URL with symbols", "false", urlvalidator, "http://www.example.com]");
	...
	/*
	 * Unit Test Start with smallest unit of url then expand Scheme + Path ex:
	 * news:comp.infosystems.www.servers.unix
	 * 
	 * Scheme + Authority + Path ex: telnet://192.0.2.16:80/
	 * 
	...
	/*
	 * Unit testing for Scheme Params: scheme[](String) - Array of schemes
	 * result(String) - value of true or false urlval(UrlValidator object)
	 */
	...
	/*
	 * Host data max of 253 ASCII characters can contain A-Z, 0-9, and - must be
	 * deliminated by . ex: en.wiki.org
	 * 
	 */
	...
	/*
	 * Testing Port make port int 0, 65535, and 70000, 1000000 make sure port starts
	 * with :
	 */
	...
	public void testIsValid() {
		...
		/*
		 * Cited: rfc3986 documentation https://tools.ietf.org/html/rfc3986 example:
		 * https://john.doe@www.example.com:123/forum/questions/?tag=networking&order=
		 * newest#top
		 * 
		 * scheme: https, http, ftp host: www.example.com port: 123 path:
		 * /forum/questions/ query: ?tag=networking&order=newest fragment: #top
		 */

		/*
		 * Scheme data https, http, ftp
		 */
		...
		/*
		 * path data beings with / or empty begins with / but not //
		 */
		...
		/*
		 * query data starts with a ? can be alphanumeric
		 */
		...
		/*
		 * Per the instructor: Programming based testing would have a list of URLs
		 * stored somewhere as a unit test asset, and the unit test would read the set
		 * of URLs in and iterate through the list until it hits an assert that fails.
		 * For a random test, the unit test would execute the randomizer and then start
		 * feeding the URLs to the isValid method.
		 */
